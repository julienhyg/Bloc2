...
Notre équipe PMO s’est inspirée de ces recommandations pour construire ses différents tableaux de bords. 
...
Après avoir définis les indicateurs clés de performance, nous avons réalisé un tableau global collectant toutes les informations des projets. 
...
Le tableau général d’état de projets permet une plus grande lisibilité et correspond à la traduction visuelle du tableau initial des chiffres. 
...
Afin d’assurer le pilotage du portefeuille de projets en tenant compte des facteurs de changements, nous avons réalisé des tableaux de synthèse ou tableaux de bords mensuels.  
...
Ainsi, par rapport aux valeurs planifiées calculées à partir des informations dont nous disposions, nous obtenant les écarts pour chaque mois d’avancement des projets. 
...
Ces chiffres d’écarts en positif ou négatif viennent alimenter automatiquement des diagrammes de courbes beaucoup plus visuels et permettant de suivre avec efficacité l’avancement des projets au cours de l’année.

Ces chiffres sont compilés indépendamment pour chaque projet afin de pouvoir réaliser des compte rendus davantage ciblés, puis regroupés dans un tableau de bord général du portefeuille.

![Exemple de tableau de bord du projet de CRM (projet 1)](ASSETS/IMAGES/5/5_exemple_tableau_bord_crm.PNG)

Pour chaque projet, nous pouvons observer en un coup d’œil et mois par mois (M0, M1…) les chiffres dépassant (en rouge) les valeurs planifiées attendues. 
Les diagrammes correspondant permettent quant à eux de constater plus clairement encore si les valeurs réelles (en foncé) dépassent les valeurs attendues (en clair) comme c’est le cas sur ce projet 1. 
...
Ce tableau pourra facilement être complété pour le restant de l’année et les valeurs d’écarts calculées automatiquement grâce au fonctions de calcul excel. 
...
Suite aux dernières études réalisées par *Time’EATS*, il semble qu’il y ait une accélération de l’implantation des concurrents sur le marché national de la livraison de plats cuisinés. 
...
D’autres part, il semble que les consommateurs ciblent désormais leurs commandes sur les applications en ligne à hauteur de 30% de plus ce qui conforte notre stratégie d’implantation numérique. 
 
Au vu du bilan réalisé ce mois de juin sur l’état des projets en cours et des résultats des études de marché de l’entreprise, une mise à jour de la priorisation des projets du portefeuille nous parait essentiel. 
Outre le niveau de maitrise des risques qu’il convient de mettre à jour lors du cycle de vie des projets, nous avons également modifié la note concernant l’avantage concurrentiel et celle de contribution à la croissance de certains projets en tenant compte de ces évolutions de marché. 
...
Ce projet connait de fortes dérives et nous proposons de le mettre en stand-by afin d’attribuer des ressources supplémentaires au projets 2 et 7. 
...
Comme prévu initialement, ce projet ne doit pas démarrer tant que nos objectifs stratégiques ne sont pas majoritairement atteints. 
 ...
 -  **Projet 6** (mise en place d’outils de cyber sécurité) : 
Projet terminé 
...
En effet, nos clients doivent pouvoir bénéficier d’ une interface ergonomiques et attractive permettant de pouvoir commander nos produits avec simplicité et en toute sécurité. 
Les données initialement prévues pour le CRM devront être mise en forme pour être analysées dans power BI ou l’un des produits choisis en attendant.  
